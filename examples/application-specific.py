...
from mirgecom.simutil import (
    ...
    float_factory, int_factory
)


class autoignition_simulation(SimulationApplication):

    def __init__(self, actx=None, comm=None, restart_file_root=None,
                 casename=None, config_file_path=None, log_manager=None, **kwargs):
        super().__init__(actx=actx, comm=comm, restart_file_root=restar_file_root,
                         casename=casename, config_file_path=config_file_path,
                         log_manager=log_manager)
        from mirgecom.simutil import global_reduce as _global_reduce
        self.all_reduce = partial(_global_reduce, comm=comm)
        self._min_prs = self._configurate("minimum_pressure", 0, int_factory)
        self._max_prs = self._configurate("maximum_pressure", np.finfo(float).max, float_factory)
        self._min_tmpr = self._configurate("minimum_temperature", 0, float_factory)
        self._max_tmpr = self._configurate("maximum_temperature", np.finfo(float).max, float_factory)
        self._order = self._configurate("element_order", 1, int_factory)

    def _configurate(self, param_name, default_value=None, parameter_factory=None):
        if param_name in self._config_params:
            if data_type:
                return parameter_factory(self._config_params[param_name])
            else:
                return self._config_params[param_name]
        return default_value

    def get_mesh(self):
        self._dim = self._configurate("dim", 2, int_param)
        char_len = self._configurate("characteristic_length", 1e-3, float_param)
        x_min = self._configurate("x_min", 0., float_factory)
        x_max = self._configurate("x_max", 1., float_factory)
        y_min = self._configurate("y_min", 0., float_factory)
        y_max = self._configurate("y_max", 1., float_factory)
        z_min = self._configurate("z_min", 0., float_factory)
        z_max = self._configurate("z_max", 1., float_factory)
        n_refine = self._configurate("refinement_factor", 1, int_factory)
    ...
    def my_get_timestep(t, dt, state):
        ...
        return app.pre_step(step, t, dt, state)
        cv, tseed = state
