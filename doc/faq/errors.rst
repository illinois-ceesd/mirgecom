...
The reason for these errors is unknown. A theory is that there is a race
condition between caching a compiled kernel on disk and the loading of that
kernel from that cache. The only known "fix" at the moment is to
